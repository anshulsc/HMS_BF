...
from Plotvib import extract_features, plot_features, envelope_plot, extract_features_time, extract_features_frequency
...
def geneticAlgo(features,Y,k):
    ...
    for i in range(population_size):
        ones_indices = np.random.choice(chromosome_length, k, replace=False)
...
def plot_cm(model, name, X_test, y_test, fs_method, le):
    ...
    confusion_matrix_percent = np.round((confusion_matrix / row_sums[:, np.newaxis]),decimals=2)
    cm_display = metrics.ConfusionMatrixDisplay(confusion_matrix=confusion_matrix_percent, display_labels=list(model.classes_) if fs_method != 'ReliefF' else list(le.classes_))

    labels = list(model.classes_)
    cm_df = pd.DataFrame(confusion_matrix_percent, index=labels, columns=labels)
...
def model_complie(df, model, fs_method, div, analyze, super_label, df_type, domain, total_segments):
    ...
    # st.write(hop_length)
     ...
     if domain == "Time Domain":
        ...
        for file in list(df.keys()):
            df_features[file] = extract_features_frequency(df[file], frame_size, hop_length)
     ...
     # Giving labels to each file
     if df_type == 'csv':
         for file in list(df.keys()):
            index = file.index('NL') if 'NL' in file else file.index('WL')
            if "CF" in file or 'OR' in file: 
                DF['labels'].extend( [f'{file[:3]}{file[4:index + 2]}{file[-2:]}'] * length )
            else:
                DF['labels'].extend( [f'{file[:index+2]}{file[-2:]}'] * length )
            if "CF" in file or 'OR' in file : 
                DF['super_labels'].extend( [f'{file[:3]}{file[4:index + 2]}_xy'] * length )
            else:
                DF['super_labels'].extend( [f'{file[:index + 2]}_xy'] * length )
            for key in df_features[list(df.keys())[0]].keys():
                 DF[key].extend(df_features[file][key])
     
     else:
        for file in list(df.keys()):
            DF['labels'].extend( [file[0:2]] * length  )
            file_ = file.split(' ')[0]
            DF['super_labels'].extend( [f'{file.split(" ")[0]}'] * length )
            for key in df_features[list(df.keys())[0]].keys():
                DF[key].extend(df_features[file][key])

     #st.write(DF)

     st.markdown( """<style>.container-with-margin {margin-top: -10px; margin-bottom: 10px;}</style>""",unsafe_allow_html=True)

     # Making dataframe
     dataframe = pd.DataFrame(DF)
    
     left, right = div.columns([0.2, 0.8], gap="large")
     if df_type == 'csv':
         
         axis =  left.selectbox("Axis",["x",'y','xy'] )
         if axis != 'xy':
             dataframe = dataframe[dataframe['labels'].str.endswith(axis)]
             super_label = False
         else:
             super_label = True
     ...
     if df_type == 'csv':
         k  = 6 if axis == 'x' or axis == 'y' else 7
     else:
         k = 7 if domain == 'Time Domain' else 5
     ...
     if fs_method == 'F-Score':
         ...
         right.write(f'Selected Features : {k} out of {len(X.columns)} features')
     ...
     if fs_method == 'OMP':
         ...
         omp.fit(X_train_scaled,y_train_le)
         params = abs(omp.coef_)
         right.write(f'Selected Features : {len(list(X.columns[params!=0]))} out of {len(X.columns)} features')
         right.write(f'Selected Features : {list(X.columns[params!=0])}')
         X_train = scaler.fit_transform(X_train.drop(columns=list(X.columns[params==0])))
         X_test =  scaler.transform(X_test.drop(columns=list(X.columns[params==0])))
    ...
    #      X_test =  X_test.loc[:,selectd_features]
     ...
     if fs_method == 'PCA':
         ...
         right.write(f' Extracted Features : {k} from {len(X.columns)} Dimension')
         X_train = pca.fit_transform(X_train_scaled, y_train)
    ...
    #      le = LabelEncoder()
    #      y = le.fit_transform(Y)
    #      X_train, X_test, y_train_fs, y_test_fs = train_test_split(X_std,y, test_size = 0.25, random_state = 0)
    #      import sklearn_relief as sr
    #      r = sr.RReliefF(n_features = 5)
    #      X_train = r.fit_transform(X_train ,y_train_fs)
    ...
    #  X_test = scaler2.transform(X_test)
     ...
     if analyze == "Confusion Matrix":
        fig = plot_cm(model, model.__class__.__name__,X_test, y_test,fs_method,le)
        with div: 
...
def main():
    ...
    if uploaded_files:
        ...
        if model:
            domain = cont2.selectbox("Feature Domain",["Time Domain",'Frquency Domain'] )
            feature_selection = cont1.checkbox("Feature Selection")
            super_label = cont1.checkbox("Super_label")
            fs_method = cont2.selectbox("Feature Selection Method",["",'F-Score','OMP',"PCA","Genetic Algorithm"] ) if feature_selection else None
